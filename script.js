...
var boxSizeArray = [6,6,6,6,6,6,6];	// Array indicating how many items there is room for in the right column ULs
var numberofAnswers = [4, 3, 5];	//number of answers for each box
...
function addCorrectMessage() {
	document.querySelector("#correctMessage").classList.remove('displayNone');
}
function removeCorrectMessage() {
	document.querySelector("#correctMessage").classList.add('displayNone');
}
...
// 	for(var no=0;no<destinationBoxes.length;no++){ //destination box = 3 (cnidarian classes)
// 		var subLis = destinationBoxes[no].getElementsByTagName('LI');
// 		if(subLis.length<boxSizeArray[no])return; //cancel	
...
function checkAnswers() {
	for(var no=0;no<destinationBoxes.length;no++){ //destination box = 3 (cnidarian classes)
		var subLis = destinationBoxes[no].getElementsByTagName('LI'); //find number of items in each box (currently)
		if(subLis.length<numberofAnswers[no])return; //cancel	
...
function initDragDropScript()
{
	...
	for(var no=0;no<uls.length;no++){
		uls[no].style.height = itemHeight * boxSizeArray[no] * 0.5 + 'px';
